...
def import_data_w_Ca(D_ppc,n,window,c_ind):    
    ...
    return X3,Y, L_all,L_all_onset, L_all_offset, stim_onset2, r_onset, Xstim

# %% glm_per_neuron function code
def glm_per_neuron(n,c_ind, fig_on):
    X, Y, Lm, L_on, L_off, stim_onset, r_onset, Xstim = import_data_w_Ca(D_ppc,n,window,c_ind)
    ...
    for n in np.arange(np.size(D_ppc,0)):
        ...
        if D_ppc[n,4][0][0] > 0:
            try:
                ...
                print("Break, no fit") 
np.save('RTnew_0411.npy', Data,allow_pickle= True)     

# Data = np.load('R2new_0130.npy', Data,allow_pickle= True).item     

 # %% plot R score 


d_list3 = good_list2 <= 179
# d_list3 = good_list <= 118

d_list = good_list2 > 179
cmap = ['tab:orange','tab:orange','tab:blue','tab:blue','tab:red','tab:red','black','green','tab:purple','tab:purple']
...
for ind in [0,1]:
    for f in np.arange(ax_sz):
        ...
        for m in np.arange(np.size(W,0)):
            ...
            #     SD = 0.05
            if max_peak[n]> 0:    
                if W[n,max_peak[n]] > 2.5*SD:
                    list1.append(m)
                    list3.append(m)
                elif W[n,max_peak[n]] <-2.5*SD:
                    list2.append(m)
                    list3.append(m)
...
for f in [4]: #np.arange(ax_sz):
    ...
    for ind in [0,1]:
        ...
        axes[ind].plot(xaxis,y1,c = cmap[f],linestyle = 'dashed', linewidth = 3)
        axes[ind].fill_between(xaxis,y1-e1,y1+e1,facecolor = cmap[f],alpha = 0.3)
        axes[ind].set_ylim([-0.1,0.7])
...
ind = 0
test = np.intersect1d(listOv[ind,4], listOv[ind,5]) 
...
axes.bar(np.arange(10)*3,b11+b12, color = cmap, edgecolor = edgec, alpha = 1, width = 0.5, linewidth = 2, hatch = '/')
